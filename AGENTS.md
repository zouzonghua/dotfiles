...
- **Shell init injection**: Appends a validated `# BEGIN ZOUZONGHUA DOTFILES … # END ZOUZONGHUA DOTFILES` block to `~/.zshrc` and `~/.bashrc` that sources `~/.config/shell/init.sh`.
